-- Email promozionale una tantum per gli account verificati non ancora iscritti.
-- Non cancella né modifica iscrizioni o pagamenti.

alter table public.registration_email_notifications
  add column if not exists promotion_deadline_email_claimed_at timestamptz,
  add column if not exists promotion_deadline_email_sent_at timestamptz;

create or replace function public.claim_promotion_deadline_emails(
...
begin
  ...
  with candidates as (
    ...
    where notifications.promotion_deadline_email_sent_at is null
      and now() <= timestamptz '2026-08-10T23:59:59+02:00'
      and (
        notifications.promotion_deadline_email_claimed_at is null
        or notifications.promotion_deadline_email_claimed_at < now() - interval '30 minutes'
      )
      and not exists (
        select 1
  ...
  set
    promotion_deadline_email_claimed_at = clock_timestamp(),
  ...
  returning
    ...
    notifications.promotion_deadline_email_claimed_at;
end;
$$;
...
begin
  if email_kind_value = 'welcome' then
    ...
    update public.registration_email_notifications
    set
      promotion_deadline_email_sent_at = now(),
...
revoke all on function public.claim_promotion_deadline_emails(integer)
...
grant execute on function public.claim_promotion_deadline_emails(integer)
  to service_role;
